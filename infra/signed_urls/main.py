"""Signed-URL minting Cloud Function (sprint-13.5 Stage 1 / job-0251).
...
the single trust boundary between "an authenticated user" and "a GCS object":
  ...
  1. Firebase ID token (Authorization: Bearer <token>) is verified server-side.
     ...
     request body's ``user_id`` MUST equal it, or we reject (never trust body).
  2. ``user_id`` ownership of ``case_id`` is checked against MongoDB Atlas (the
     ``projects`` collection — Case <-> projects 1:1 per FR-MP-5).
  3. Only then do we mint a GCS V4 signed URL for the layer object, with TTL
...
an MCP stdio sidecar:
  ...
  - The ownership read is a single ``find_one({_id: case_id})`` — a direct
    PyMongo read against the Atlas URI (from Secret Manager) is the minimal,
    auditable surface. We REUSE the translator's logical contract exactly:
    the same collection name (``projects``), the same primary key (``_id`` ==
    ``case_id``), and the same user-link semantics (``user_id`` OR
    ``owner_user_id``) that ``Persistence.list_cases_for_user`` filters on.
  ...
  mintable by an arbitrary signed-in user.
...
#: MongoDB database + collection — pinned to the same nomenclature the agent's
#: Persistence layer uses (persistence.py: CASES_COLLECTION = "projects",
#: Case primary key is the ``_id`` == ``case_id``).
...
def _prod_fetch_case_doc(case_id: str) -> Optional[dict[str, Any]]:
    ...
    a short-lived function reads the single owning document directly rather than
    spawning a Node MCP sidecar. Same collection + key + user-link contract.
    """
    ...
    try:
        coll = client[DEFAULT_DATABASE][CASES_COLLECTION]
        return coll.find_one({"_id": case_id})
    finally:
        client.close()
...
def mint_signed_url(
    ...
    *,
    ...
    Args:
        ...
        user_id: the user_id claimed in the request body.
    ...
    Raises:
        BadRequest / Unauthorized / Forbidden / NotFound on validation/auth/
        ownership failures (each carries an HTTP ``status``).
    """
    d = deps or _resolve()
...
def handle_request(request: Any) -> tuple[str, int, dict[str, str]]:
    ...
    ``Authorization: Bearer <firebase-id-token>`` header. Cloud Run / Functions
    gen2 with ``--no-allow-unauthenticated`` is the OUTER gate (IAM invoker);
    the Firebase token check here is the application-level identity gate.
