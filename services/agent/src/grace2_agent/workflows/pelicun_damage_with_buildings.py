...
``GRACE2_TEST_LIVE_PELICUN_V2=1`` guard).
...
logger = logging.getLogger("grace2_agent.workflows.pelicun_damage_with_buildings")
...
def density_cog_to_point_fgb(cog_uri: str) -> str:
    ...
    tmp = _tempfile.NamedTemporaryFile(suffix=".fgb", delete=False, prefix="grace2_density_pts_")
    tmp_path = tmp.name
    tmp.close()
