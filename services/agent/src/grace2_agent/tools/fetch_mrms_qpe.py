...
*Geosci. Model Dev.* (2025) — this is the precipitation product the SFINCS
pluvial reference workflow expects. Together with the ``fetch_mrms_reflectivity``
sibling (job-0102) the pair covers MRMS reflectivity (radar product, mm/hr) and
QPE (gauge-corrected accumulation product, mm).

API surface (verified 2026-06-08, AWS Open Data Registry):

    bucket:   noaa-mrms-pds  (https://noaa-mrms-pds.s3.amazonaws.com/)
    prefix:   CONUS/MultiSensor_QPE_<ACCUM>_Pass2_00.00/YYYYMMDD/
    filename: MRMS_MultiSensor_QPE_<ACCUM>_Pass2_00.00_YYYYMMDD-HHMMSS.grib2.gz

    where <ACCUM> ∈ {01H, 03H, 06H, 12H, 24H, 48H, 72H}.

We use the ``Pass2`` (delayed, gauge-corrected) product for cache-stable
historic queries; ``Pass1`` (real-time, radar-only) is available under the
parallel ``Pass1`` prefix and surfaced as ``OQ-0103-MRMS-PASS-CHOICE`` for a
future explicit-pass kwarg.

File layout (verified live):
    - ~3-5 MiB grib2.gz per file
    - CONUS 0.01° grid: shape (3500, 7000), bounds (-130, 20, -60, 55)
    - GRIB sphere CRS (close to EPSG:4326 within ~1e-3°); we reproject
      output to EPSG:4326 via rasterio's ``WarpedVRT`` for CRS hygiene
    - Native dtype float64, values in mm
    - MRMS sentinel values: ``-3.0`` = no-precip, ``-1.0`` = missing/masked.
      We convert both to GeoTIFF nodata (-9999.0 sentinel) so downstream
      consumers can mask cleanly.

FR-TA-2 atomic tool. FR-CE-8 / FR-DC-3/4: routed through ``read_through`` so
identical ``(bbox, accumulation, valid_time)`` calls reuse the cached GeoTIFF.

Cache key composition (per kickoff): SHA-256 of ``(bbox-rounded-6dp,
accumulation, valid_time-quantized)``. Recent data (within the dynamic-1h
TTL window) lands under ``cache/dynamic-1h/mrms_qpe/<hash>.tif``; older
historic queries are bucketed by the dynamic-1h vintage too — the kickoff's
suggestion of a static-30d branch for historic data is surfaced as
``OQ-0103-MRMS-HISTORIC-TTL`` (a future schema enrichment, not landed here
because the current ``read_through`` is one-TTL-per-tool).

Tier-1 (free; no auth). CRS-tagged GeoTIFF output; bbox in EPSG:4326.
"""
...
def fetch_mrms_qpe(
    bbox: tuple[float, float, float, float] | None = None,
    accumulation: Literal["1h", "3h", "6h", "12h", "24h", "48h", "72h"] = "24h",
    valid_time: str | None = None,
    # job-0164: absorb LLM-invented kwargs (centralized at server.py via
    # tool_arg_normalizer, but kept as belt-and-suspenders).
    **_extra_ignored: Any,
) -> LayerURI:
    """NOAA MRMS accumulated QPE -- gauge-corrected radar precipitation (mm) COG.

    Downloads the NOAA MRMS MultiSensor QPE Pass2 product from the public
    ``noaa-mrms-pds`` S3 bucket (anonymous), decodes + clips to bbox, reprojects
    to EPSG:4326, and writes a deflate COG of accumulated precipitation in mm.
    Pass2 is gauge-corrected (~2 h delayed) at CONUS 0.01deg (~1 km). Tier-1
    free. Omit ``valid_time`` for the most-recent published file.

    Use this when:
    - SFINCS pluvial-flood forcing precipitation for CONUS events (MRMS QPE is
      the Harvey/Houston SFINCS reference forcing).
    - Rainfall-runoff / storm totals ("how much rain fell in 24 h over the
      watershed?").
    - Feeding ``model_flood_scenario(forcing_raster_uri=...)`` as real-precip
      forcing.

    Do NOT use this for:
    - Live radar reflectivity -- use ``fetch_nexrad_reflectivity`` (dBZ, not mm).
    - Design-storm return periods -- use ``lookup_precip_return_period`` (NOAA
      Atlas 14).
    - Precip OUTSIDE CONUS -- MRMS is CONUS-only; use
      ``fetch_chirps_precipitation`` (quasi-global daily) or ``fetch_gridmet``
      (CONUS daily met) or ``fetch_era5_reanalysis`` (global ERA5).

    Returns a raster LayerURI (role="primary", units="mm") that auto-renders --
    do not call publish_layer. Must intersect CONUS ``(-130, 20, -60, 55)`` or
    ``MRMSQPEEmptyError`` is raised.

    Parameters:
    - ``bbox``: ``(min_lon, min_lat, max_lon, max_lat)`` EPSG:4326; None = full
      CONUS grid (~3500x7000 px).
    - ``accumulation``: window -- ``"1h"``, ``"6h"``, ``"24h"`` (default),
      ``"72h"`` (also ``"3h"`` / ``"12h"`` / ``"48h"``; uppercase S3 tokens like
      ``"24H"`` accepted too). ``"1h"`` is the finest window.
    - ``valid_time``: ISO-8601 UTC (e.g. ``"2017-08-27T12:00:00Z"``); None =
      most-recent. Resolves to the nearest-earlier published hour within a 24 h
      walkback (Pass2 ~2 h delayed).

    Cached ``dynamic-1h``. GeoTIFF float32, EPSG:4326, nodata=-9999, band
    "precipitation_mm"; the name embeds the resolved valid_time (provenance).
    Raises MRMSQPEInputError / MRMSQPEUpstreamError / MRMSQPENotAvailableError /
    MRMSQPEEmptyError.
    """
