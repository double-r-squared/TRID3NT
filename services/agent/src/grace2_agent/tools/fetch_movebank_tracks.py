...
logger = logging.getLogger("grace2_agent.tools.fetch_movebank_tracks")
...
def _resolve_credentials(
    ...
    3. ``GRACE2_MOVEBANK_USER`` + ``GRACE2_MOVEBANK_PASSWORD`` env vars (local
    ...
    # 3. env-var fallback.
    if resolved_user is None:
        resolved_user = os.environ.get("GRACE2_MOVEBANK_USER")
    if resolved_pass is None:
        resolved_pass = os.environ.get("GRACE2_MOVEBANK_PASSWORD")

    if not resolved_user or not resolved_pass:
        raise MovebankInputError(
            "Movebank credentials missing: pass (username + password) OR "
            "secret_ref OR set GRACE2_MOVEBANK_USER + GRACE2_MOVEBANK_PASSWORD"
        )
...
def _records_to_flatgeobuf_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_movebank_"
        ) as fgb_f:
            tmp_fgb = fgb_f.name
        try:
