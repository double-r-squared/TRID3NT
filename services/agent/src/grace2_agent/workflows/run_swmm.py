...
module owns the urban-flood engine's solver-dispatch surface:
  ...
  3. **A SWMM ``LocalSolverSpec``** (``swmm_local_spec``) + ``'swmm'`` registered
     ...
     staged-manifest local-backend path (``GRACE2_SOLVER_BACKEND=local-docker``)
...
logger = logging.getLogger("grace2_agent.workflows.run_swmm")
...
def is_local_mode() -> bool:
    ...
    LOCAL lane is the DEFAULT (``GRACE2_SWMM_LOCAL`` unset -> local). Set
    ``GRACE2_SWMM_LOCAL=0`` to force the out-of-process staged-manifest dispatch
    ...
    external substrate.
    """
    raw = os.environ.get("GRACE2_SWMM_LOCAL")
...
def swmm_local_spec() -> Any:
    ...
    ``GRACE2_SOLVER_BACKEND=local-docker`` if a sandboxed worker lane is wired.
    """
    import sys
    ...
    def classify_exit(
        ...
        tol = 5.0
        try:
            tol = float(os.environ.get("GRACE2_SWMM_MASS_BALANCE_TOL_PCT", "5.0"))
