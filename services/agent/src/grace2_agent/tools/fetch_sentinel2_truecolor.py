...
Fetches a Sentinel-2 Level-2A true-color (natural-color) image for a bbox + a
recent date window via the Microsoft Planetary Computer (PC) STAC catalog and
returns a 3-band RGB COG that paints directly as a recent satellite basemap.

True color combines the visible bands:

    Red   = B04 (665 nm, 10 m)
    Green = B03 (560 nm, 10 m)
    Blue  = B02 (490 nm, 10 m)

Unlike NAIP (US-only, ~1 m, multi-year cadence) this is GLOBAL and refreshed
every ~5 days, so it is the go-to "what does this area look like right now from
space" layer anywhere on Earth.

Data source
===========

PC collection ``sentinel-2-l2a`` (Sentinel-2 Level-2A, 10 m surface
reflectance):

    catalog: https://planetarycomputer.microsoft.com/api/stac/v1
    bands:   B04 / B03 / B02 (visible, 10 m) + SCL (scene classification, 20 m)
...
The SCL (Scene Classification Layer) band labels each pixel. We mask the
cloud / shadow / saturated / nodata classes ``{0,1,3,8,9,10}`` (no-data,
saturated-or-defective, cloud-shadow, cloud-medium-probability,
cloud-high-probability, thin-cirrus) so clouds do not pollute the contrast
stretch and are emitted as black no-data rather than blown-out white.

True-color rendering
====================

Sentinel-2 reflectance is uint16 (~0-10000+, not 0-255), so the three bands are
jointly percentile-stretched (2nd..98th over the CLEAR pixels) to uint8 and
baked into a 3-band RGB COG. ``publish_layer`` renders multiband (>=3 band) COGs
directly via the RGBA/multiband passthrough, so no per-band rescale / colormap
is applied  --  the baked natural colors paint as-is. ``style_preset`` is the
multiband-passthrough token ``"s2_truecolor"`` (intentionally NOT in the
single-band TiTiler style registry).
...
``S2TrueColorNoImageryError`` is raised  --  never a fabricated layer.

FR-CE-8 / FR-DC-3/4: routed through ``read_through`` so identical
``(bbox, window, max_cloud_cover)`` calls reuse the cached RGB COG in the
``static-30d`` / ``s2_truecolor`` cache prefix.
...
logger = logging.getLogger("grace2_agent.tools.fetch_sentinel2_truecolor")
...
def _fetch_truecolor_cog_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".tif", delete=False, prefix="grace2_s2tc_"
        ) as f:
            tmp_path = f.name
        profile = dict(
            driver="COG",
