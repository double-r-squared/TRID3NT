...
Wraps the NOAA HRRR-Smoke smoke / aerosol forecast distributed through the
University of Utah CHPC HRRR-Zarr mirror on the public anonymous AWS S3
bucket ``s3://hrrrzarr/``. Returns a CRS-tagged GeoTIFF (EPSG:4326) carrying
a single smoke / aerosol field — near-surface smoke mass density, vertically
integrated smoke column mass, or aerosol optical thickness — at a single
forecast lead time, reprojected from HRRR's native Lambert Conformal Conic
grid and clipped to the requested CONUS bbox.

HRRR-Smoke is NCEP/EMC's online fire-emissions extension of HRRR (NOAA Air
Resources Laboratory + NOAA/GSL, operational 2020). Wildfire emissions are
ingested in near-real time from the NESDIS Hazard Mapping System (HMS)
satellite fire detections (GOES-East + GOES-West + polar orbiters), injected
through a simplified online plume-rise scheme (Freitas et al., 2006), and
advected by the same convection-resolving 3 km HRRR dynamical core that
drives the standard HRRR forecast. The smoke fields are co-emitted into the
same hourly HRRR cycle (00z, 06z, 12z, 18z extended to 48 h; all other
cycles 18 h horizon) and live in the same ``hrrrzarr`` bucket as standard
HRRR.

Supported variables (single-level diagnostics; all available hourly):

    Variable name           Group                         S3 var   Units      Description
    -------------------------------------------------------------------------------------
    "near_surface_smoke"    8m_above_ground               MASSDEN  kg m-3     8 m AGL smoke mass density
    "smoke_column_mass"     entire_atmosphere_single_layer COLMD   kg m-2     Vertically-integrated smoke column
    "aerosol_optical_depth" entire_atmosphere_single_layer AOTK    1          550 nm aerosol optical thickness

(MASSDEN is the GRIB2 mass-density of "tracer #1" — the HRRR-Smoke aerosol
mixing ratio interpreted at 8 m AGL. COLMD is the vertically-summed mass
of that tracer per unit horizontal area. AOTK is the HRRR-internal
diagnostic optical depth, dimensionless.)

HRRR projection (identical to standard HRRR — constant across every cycle):
...
y ∈ [-1 587 306, +1 586 694] m at 3 km spacing — covers CONUS plus the
Great Lakes / immediate offshore. Outputs are reprojected to EPSG:4326
via ``rioxarray.reproject`` before clipping to bbox.

Mirror layout (verified 2026-06-09 against live S3):

    bucket prefix:   ``s3://hrrrzarr/sfc/<YYYYMMDD>/<YYYYMMDD>_<HH>z_fcst.zarr/``
    level group:     ``<level>/`` — ``8m_above_ground`` or
                     ``entire_atmosphere_single_layer``
    variable group:  ``<level>/<VAR>/`` — coord arrays (time,
    ...
    data array:      ``<level>/<VAR>/<level>/<VAR>`` — float64 chunked array
                     dims (time, projection_y_coordinate, projection_x_coordinate)
                     shape (48, 1059, 1799), fill_value=-9999.0

The data lives in a doubly-nested group (same shape as the standard HRRR
mirror): opening the outer ``<level>/<VAR>`` group gives the coord arrays
but no data; opening the inner ``<level>/<VAR>/<level>/`` group gives a
Dataset whose only ``data_var`` is the float64 array. We merge the two
with ``xarray.merge(compat="override")``.

Cycle latency mirrors standard HRRR: a given ``<HH>z`` cycle's smoke
forecast publishes ~1.0–1.5 h after the cycle hour. The fetcher selects
the most-recent cycle whose forecast at the requested ``forecast_hour``
is already on S3, walking backward in 1-h steps if the requested cycle
is not yet posted.

FR-TA-2 atomic tool. FR-CE-8 / FR-DC-3/4: routed through ``read_through``
with ``ttl_class="dynamic-1h"`` — HRRR-Smoke updates hourly and an
in-flight forecast supersedes any prior cycle's same-valid-time slice
within ~1 h.
...
predicting the same valid time map to distinct cache entries.

``supports_global_query=False`` — HRRR-Smoke is CONUS-only. A bbox outside
the CONUS LCC extent raises ``HRRRSmokeInputError``.

Tier-1 (free; no auth). NOAA Big Data Program S3 anonymous access.

Composes with: ``fetch_nifc_fire_perimeters`` (active wildfire polygons),
``fetch_firms_active_fire`` (satellite hot spots), ``fetch_mtbs_burn_severity``
(historical burn footprints), ``fetch_asos_metar`` (surface obs for air-
quality context), ``fetch_gridmet`` (fire-weather inputs). Layered with
NIFC perimeters or FIRMS detections, this tool produces the canonical
"smoke plume from the wildfire" demo overlay.
"""
...
logger = logging.getLogger("grace2_agent.tools.fetch_hrrr_smoke")
...
def _zarr_slice_to_geotiff_bytes(
    ...
    try:
        ...
        outer_mapper = fsspec.get_mapper(outer_path, **public_s3fs_kwargs())
        inner_mapper = fsspec.get_mapper(inner_path, **public_s3fs_kwargs())
    ...
    try:
        ...
        # Write COG.
        out_fd, out_path = tempfile.mkstemp(suffix=".tif", prefix="grace2_hrrr_smoke_")
        os.close(out_fd)
        try:
...
def fetch_hrrr_smoke(
    ...
    **_extra_ignored: Any,
    ...
    try:
        from ._public_s3 import public_s3fs_kwargs
        fs = fsspec.filesystem("s3", **public_s3fs_kwargs())
    except Exception as exc:  # noqa: BLE001
        raise HRRRSmokeUpstreamError(
