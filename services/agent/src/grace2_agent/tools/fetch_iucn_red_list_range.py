"""``fetch_iucn_red_list_range`` atomic tool — IUCN Red List range info Tier-2 fetcher (job-0129).

Wraps the IUCN Red List API v3 (``https://apiv3.iucnredlist.org/api/v3``) to
return species range *INFO* (threat assessment + range geometry placeholder)
for a given scientific name as a FlatGeobuf. This is the v0.1 stance for the
IUCN substrate: the SimpleAPI exposes the per-species assessment metadata
(category, criteria, population trend, regional/global scope) and a
``species/region/{name}/{region}`` endpoint, but the actual range polygons
require a separate Spatial Data download flow that is out of scope here.

See ``OQ-0129-RANGE-SPATIAL`` (TENTATIVE): the v0.1 surface returns a
single-feature FlatGeobuf carrying the threat-assessment payload with a
placeholder polygon geometry built from the species' ``elevation_lower``/
``elevation_upper`` /  occurrence-region hints; the live polygon ingest will
land later via an IUCN Spatial Data fetcher or a wrapper around the
``https://geospatial.iucnredlist.org/`` ArcGIS tiles. This surface lets
downstream conservation workflows (Case 1) consume IUCN threat status today
without blocking on the Spatial Data pipeline.

API surface (verified 2026-06-08 against IUCN Red List API v3 documentation
at ``https://apiv3.iucnredlist.org/api/v3/docs``):

    SimpleAPI species lookup (global):
        https://apiv3.iucnredlist.org/api/v3/species/{species_name}?token={api_key}

    Regional assessment:
        https://apiv3.iucnredlist.org/api/v3/species/region/{species_name}/{region}?token={api_key}
...
Tier-2 keyed: requires an IUCN Red List API key (free for research use; see
``https://apiv3.iucnredlist.org/api/v3/token``). The kickoff (job-0129)
specifies the three accepted paths:

1. ``api_key: str`` explicit param (CLI / direct invocation).
2. ``secret_ref: SecretRecord`` looked up via ``Persistence.get_secret_value``
   — the per-Case keyed fetcher path landed by sibling job-0124.
3. ``os.environ.get("GRACE2_IUCN_RED_LIST_API_KEY")`` fallback for local dev.

If none of the three resolve a non-empty key, the tool raises
``IUCNAuthError`` BEFORE issuing any network call.

FR-TA-2 atomic tool. FR-CE-8 / FR-DC-3/4: routed through ``read_through``
keyed on ``(species_name_lower, region)`` so identical lookups reuse the
cached FlatGeobuf. The api_key value itself is NEVER part of the cache key
(two callers with different keys for the same species hit the same cache
entry — the cache stores the public response, not credentials).

Output FlatGeobuf schema (1 feature per species lookup):
    Geometry: Polygon — PLACEHOLDER (a small WGS84 square centred at the
        species' nominal range centroid or the requested region centroid).
        OQ-0129-RANGE-SPATIAL captures the v0.2 swap-in for real range
        polygons.
    Properties (per IUCN species assessment):
        taxonid                  (int)   — IUCN taxon ID
        scientific_name          (str)
        common_name              (str)   — ``main_common_name`` or ``""``
        kingdom / phylum / class / order_name / family  (str)
        category                 (str)   — IUCN threat category (e.g. "LC",
            "VU", "EN", "CR", "EX", "DD", "NT", "LR/lc")
        criteria                 (str)   — assessment criteria string
        population_trend         (str)   — "Decreasing"|"Stable"|"Increasing"|"Unknown"
        marine_system            (bool)
        freshwater_system        (bool)
        terrestrial_system       (bool)
        elevation_lower          (float|None)
        elevation_upper          (float|None)
        depth_lower              (float|None)
        depth_upper              (float|None)
        published_year           (int|None)
        assessment_date          (str|None)
        region                   (str)   — "global" by default
        is_placeholder_geometry  (bool)  — always True at v0.1; flips False
            when the Spatial Data ingest lands.

CRS: EPSG:4326 (placeholder geometry).
"""
...
def _payload_to_flatgeobuf_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_iucn_"
        ) as fgb_f:
            tmp_fgb = fgb_f.name
        try:
