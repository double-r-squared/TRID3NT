...
see reports/design/spike_canopy_height_tool.md):
  ...
  3. Dispatch through the generic ``run_solver('canopy', model_setup_uri=<build
     ...
     resolves from ``GRACE2_AWS_BATCH_JOB_DEF_CANOPY`` and stays INERT (honest
...
logger = logging.getLogger("grace2_agent.tools.compute_canopy_height")
...
#: ``"canopy"`` entry in ``tools.solver.SOLVER_WORKFLOW_REGISTRY``).
...
def _max_bbox_deg2() -> float:
    raw = (os.environ.get("GRACE2_CANOPY_MAX_BBOX_DEG2") or "").strip()
    try:
        v = float(raw)
...
async def _run_canopy_chain(
    *,
    ...
    from .solver import run_solver, select_compute_class, wait_for_completion
