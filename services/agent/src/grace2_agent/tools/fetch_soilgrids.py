...
Fetches the ISRIC **SoilGrids 2.0** global ~250 m soil-property prediction for a
bbox: a chosen soil property (clay / sand / silt content, soil organic carbon,
bulk density, pH) at one of six GlobalSoilMap standard depth intervals, and
returns a single-band float32 COG in physical units (clay/sand/silt %, soc g/kg,
bdod kg/dm3, phh2o pH).

This is the GLOBAL soil substrate for hydrology, agriculture, and infiltration
work -- the worldwide complement to the US-only STATSGO/SSURGO chain
(``fetch_statsgo_soils``). SoilGrids covers every land mass on Earth at a
consistent 250 m grid, so it answers "what is the soil texture / organic carbon /
pH / bulk density here?" anywhere (Africa, Asia, South America, Europe), not just
CONUS.

Data source (Tier-1 free, no auth required, CC-BY 4.0)
======================================================

ISRIC SoilGrids 2.0 master VRT mosaics:

    https://files.isric.org/soilgrids/latest/data/<property>/<property>_<depth>_mean.vrt

Each map is a global Int16 raster in the **Interrupted Goode Homolosine**
projection (the dataset's native equal-area CRS), at 250 m, with NoData
``-32768``, plus an ``.ovr`` overview and a folder of GeoTIFF tiles that the VRT
mosaics. We open the master VRT through GDAL ``/vsicurl/`` (HTTP byte-range), so
only the tiles overlapping the bbox are fetched -- no 5 GB whole-property
download.

Property scaling (ISRIC SoilGrids FAQ "conversion factors"; the stored Int16 is
a fixed-point encoding, divide to recover physical units):

    clay / sand / silt : g/kg   stored x10   -> divide by 10  -> %
    soc (soil org. C)  : dg/kg  stored x10   -> divide by 10  -> g/kg
    bdod (bulk density): cg/cm3 stored x100  -> divide by 100 -> kg/dm3
    phh2o (pH in H2O)  : pH x10               -> divide by 10  -> pH

Depths (GlobalSoilMap IUSS standard intervals):
...
1. Validate bbox (required -- ``supports_global_query=False``; the global mosaic
   is ~5 GB per property/depth), property, and depth.
2. Open the per-(property, depth) master VRT via ``/vsicurl/``.
3. Transform the EPSG:4326 bbox into the source Homolosine CRS and window-read
   only the overlapping native pixels (a tiny array for an AOI-scoped bbox).
4. Reproject that small native window to an EPSG:4326 target grid at ~250 m
   (bilinear; continuous data).
5. Apply the property scaling to recover physical units, write a single-band
   float32 COG (NoData ``-9999``) into the FR-DC cache (``static-30d``,
   ``source_class="soilgrids"``).

Why native-window-then-reproject (not a full ``WarpedVRT`` read): the source VRT
is 159246 x 58034; warping the whole thing scans every tile source. Transforming
the bbox into Homolosine first lets us window-read just the handful of native
pixels that matter, then reproject that small array -- correct AND it keeps the
byte-range footprint to the overlapping tiles.

Honesty (data-source fallback norm): an all-NoData window (bbox over open ocean
/ off the soil land surface) raises a typed ``SoilGridsEmptyError``; an unknown
property/depth raises ``SoilGridsInputError``; a vsicurl/rasterio failure raises
``SoilGridsUpstreamError`` -- never a fabricated layer.
...
logger = logging.getLogger("grace2_agent.tools.fetch_soilgrids")
...
def _fetch_soilgrids_bytes(
    ...
    try:
        ...
        out_fd, out_path = tempfile.mkstemp(suffix=".tif", prefix="grace2_soilgrids_")
        os.close(out_fd)
        try:
            profile = dict(
                driver="COG",
