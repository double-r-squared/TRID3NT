...
2. ``secret_ref`` (a ``SecretRecord``) -> ``Persistence.get_secret_value`` (the
   ...
   retry).
3. ``GRACE2_AIRNOW_API_KEY`` env var (dev convenience).
...
logger = logging.getLogger("grace2_agent.tools.fetch_airnow_air_quality")
...
_AIRNOW_KEY_ENV = "GRACE2_AIRNOW_API_KEY"
...
def _resolve_api_key(
    ...
    raise AirNowMissingKeyError(
        ...
        "set the GRACE2_AIRNOW_API_KEY env var. Register a free key at "
...
def _records_to_fgb(records: list[dict[str, Any]]) -> bytes:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_airnow_aq_"
        ) as f:
            tmp_fgb = f.name
        try:
