...
**API surface** (verified live 2026-06-27):
    ...
    Observed currents (product="currents", one request per station):
        https://api.tidesandcurrents.noaa.gov/api/prod/datagetter
            ...
            &units=english&application=grace2&format=json
        Response data[] rows:
...
logger = logging.getLogger("grace2_agent.tools.fetch_noaa_coops_currents")
...
def _build_flatgeobuf(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_coops_currents_"
        ) as f:
            tmp_fgb = f.name
        try:
