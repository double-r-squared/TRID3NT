...
logger = logging.getLogger("grace2_agent.tools.cut_features_with_polygon")
...
def _resolve_layer_to_local_path(
    ...
    if uri.startswith("s3://"):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=suffix, delete=False, prefix="grace2_cut_"
        ) as tmp:
            tmp.write(data)
...
def _cut_locally(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_cut_out_"
        ) as out_f:
            tmp_out = out_f.name
