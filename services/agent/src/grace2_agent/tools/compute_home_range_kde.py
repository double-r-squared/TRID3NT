...
from .cache import CACHE_BUCKET, read_through, read_object_bytes_s3
...
logger = logging.getLogger("grace2_agent.tools.compute_home_range_kde")
...
def _download_points_to_local(points_uri: str) -> str:
    ...
    if points_uri.startswith("s3://"):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=suffix, delete=False, prefix="grace2_homerange_"
        ) as f:
            f.write(data)
            return f.name
...
def _compute_home_range_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_homerange_out_"
        ) as f:
            out_tmp = f.name
...
def compute_home_range_kde(
    ...
    **_extra_ignored: Any,
    ...
    """Kernel-density home range (utilization-distribution isopleths) from track points. [track analysis]

    Use this when:
    - The user asks for an animal's "home range", "core area", "utilization
      distribution", "KDE range", or a "50%/95% isopleth" from tracking data.
    - Summarising a cloud of GPS fixes into a smooth occupancy boundary.

    Do NOT use this for:
    - Fetching the tracks -- get ``points_uri`` from
      ``fetch_movebank_tracks(geometry_type="point")`` first.
    - Speed / turning / path-length metrics -- use ``compute_movement_trajectory``
      (this is occupancy, not motion).
    - Per-zone raster statistics -- feed this output as the zone polygon to
      ``compute_zonal_statistics``.

    Honesty: too-few fixes or a degenerate co-linear / coincident set raises a
    typed ``HomeRangeKDEError(TOO_FEW_POINTS)`` -- never a fabricated polygon.
    Deterministic local compute (no external network).

    Returns a vector LayerURI that auto-renders -- do not call publish_layer.

    Fits a 2-D Gaussian kernel density over the fixes and contours the utilization
    distribution at each isopleth percentile (95% = home-range boundary, 50% =
    core-use area).

    Params:
        points_uri: ``s3://`` URI or local path of a telemetry-fix FlatGeobuf,
            typically ``fetch_movebank_tracks(..., geometry_type="point").uri``
            (Point/MultiPoint/LineString accepted; lines exploded to vertices).
        isopleths: UD-volume percentile(s) to contour -- a list like ``[50, 95]``
            (default) or a single number, each in ``(0, 100]``.
        bandwidth_m: kernel bandwidth in METRES; None (default) = Scott's rule.
        grid_size: UD grid resolution per axis (default 200, clamped [32, 600]).
        individual_id: optional -- restrict to one animal by ``individual_id``;
            None pools all fixes.

    Output features carry ``isopleth_pct``, ``area_km2``, ``n_points``,
    ``individual_id``, ``bandwidth_m``; ``bbox`` set to the isopleth extent for
    camera fly-to. ``units="km2"``. Cached static-30d. Raises typed
    ``HomeRangeKDEError`` (NO_POINTS_INPUT / DOWNLOAD_FAILED / EMPTY_LAYER /
    TOO_FEW_POINTS / BAD_ISOPLETH / BAD_BANDWIDTH / NO_ISOPLETHS / KDE_FAILED).
    """
