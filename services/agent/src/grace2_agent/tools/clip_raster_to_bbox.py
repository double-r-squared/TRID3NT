...
logger = logging.getLogger("grace2_agent.tools.clip_raster_to_bbox")
...
# ``GRACE2_GDAL_TRANSLATE_BIN`` and ``GRACE2_GDALWARP_BIN``.
...
def _get_gdal_translate_bin() -> str:
    ...
    candidate = (
        os.environ.get("GRACE2_GDAL_TRANSLATE_BIN")
    ...
    if candidate is None or not os.path.isfile(candidate):
        raise ClipRasterError(
            ...
            "gdal_translate binary not found on PATH; set GRACE2_GDAL_TRANSLATE_BIN "
...
def _get_gdalwarp_bin() -> str:
    ...
    candidate = (
        os.environ.get("GRACE2_GDALWARP_BIN")
    ...
    if candidate is None or not os.path.isfile(candidate):
        raise ClipRasterError(
            ...
            "gdalwarp binary not found on PATH; set GRACE2_GDALWARP_BIN "
            "or install gdal-bin / activate the grace2 conda env.",
        )
...
def _gdal_subprocess_env() -> dict[str, str]:
    ...
    Resolution priority (GRACE2_PROJ_LIB env var → system path → no override):
    - ``GRACE2_PROJ_LIB`` — explicit operator override (e.g. in CI or containers
    ...
    override = os.environ.get("GRACE2_PROJ_LIB")
    if override:
