...
from .compute_hillshade import _download_dem_bytes, _gdaldem_subprocess_env
...
logger = logging.getLogger("grace2_agent.tools.compute_contours")
...
def _get_gdal_contour_bin() -> str:
    ...
    1. ``GRACE2_GDAL_CONTOUR_BIN`` explicit override.
    2. The sibling of the resolved ``gdaldem`` binary (so a single
       ``GRACE2_GDALDEM_BIN`` override covers both tools).
    ...
    # GRACE2_GDALDEM_BIN override (or the conda-env fallback) covers both.
    sibling: str | None = None
    try:
        from .compute_hillshade import _get_gdaldem_bin
    ...
    candidate = (
        os.environ.get("GRACE2_GDAL_CONTOUR_BIN")
        or sibling
    ...
    if candidate is None or not os.path.isfile(candidate):
        raise ContourComputeError(
            ...
            "GRACE2_GDAL_CONTOUR_BIN (or GRACE2_GDALDEM_BIN — gdal_contour is "
