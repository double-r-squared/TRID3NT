...
(``grace2_contracts.secrets.CredentialRequestEnvelopePayload``) so the web
...
closed ``ProviderID`` Literal in ``grace2_contracts.secrets`` (the schema
...
# a member of the closed ``ProviderID`` Literal in ``grace2_contracts.secrets``
...
CREDENTIAL_PROVIDERS: dict[str, CredentialProvider] = {
    ...
    # Copernicus CDS — ONE key (GRACE2_COPERNICUS_CDS_API_KEY) serves BOTH the
    ...
    "iucn_red_list": CredentialProvider(
        ...
        secret_key_name="GRACE2_IUCN_RED_LIST_API_KEY",
        default_message=(
...
def is_credential_error(tool_name: str, error: BaseException) -> bool:
    ...
    tool with no provider returns ``False`` — the server cannot request a key
    for an unknown provider, so it narrates the failure honestly via the normal
    typed-error surface rather than fabricating a provider/prompt.
    """
