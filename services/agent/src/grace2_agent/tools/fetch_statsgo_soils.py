...
logger = logging.getLogger("grace2_agent.tools.fetch_statsgo_soils")
...
def _fetch_statsgo_field_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".tif", delete=False, prefix="grace2_statsgo_in_"
        ) as f:
            tmp_in = f.name
        try:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".tif", delete=False, prefix="grace2_statsgo_cog_"
        ) as f:
            tmp_cog = f.name
        try:
            da.rio.to_raster(
