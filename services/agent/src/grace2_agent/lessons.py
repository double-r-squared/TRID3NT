...
- **Store**: JSONL at ``GRACE2_LESSONS_PATH`` (default: ``lessons.jsonl`` under
  the file-persistence dir, ``/tmp/grace2_lessons.jsonl`` fallback -- same
...
- **Guardrails**: ADVISORY TEXT ONLY (never mutates schemas or tool behavior);
  ...
  gate ``GRACE2_LESSONS=off|on`` -- DEFAULT OFF, dark until the pass-3 harness
...
logger = logging.getLogger("grace2_agent.lessons")
...
LESSONS_PATH_ENV = "GRACE2_LESSONS_PATH"
_FALLBACK_LESSONS_PATH = "/tmp/grace2_lessons.jsonl"
...
def lessons_enabled() -> bool:
    """Live env read (never cached at import) -- ``GRACE2_LESSONS=on`` only."""
...
def _resolve_lessons_path() -> str:
    ...
    Precedence: ``GRACE2_LESSONS_PATH`` env > ``<persistence dir>/lessons.jsonl``
...
# ``GRACE2_LESSONS_PATH`` + ``reset_lesson_store()``.
...
def reset_lesson_store() -> None:
    """Drop the singleton (tests: set ``GRACE2_LESSONS_PATH`` then reset)."""
    global _STORE
    with _STORE_LOCK:
        _STORE = None
...
def observe_turn(user_text: str, calls: list[dict]) -> int:
    ...
    ``[{tool, args, success, error_code}, ...]``. Gated on ``GRACE2_LESSONS``
...
def register_lesson(text: str, trigger_text: str = "") -> dict:
    ...
    NOT gated on ``GRACE2_LESSONS`` -- an explicit user correction is never
