...
def compute_slope(
    ...
    **_extra_ignored: Any,
    ...
    """Compute terrain SLOPE (steepness in degrees or percent grade) from a DEM [terrain/DEM-derived raster].

    Use this when:
    - The user wants slope, steepness, gradient, or percent grade (landslide /
      erosion risk, road grade, accessibility, construction site assessment).
    - You need a slope raster to feed ``compute_zonal_statistics`` (e.g. mean
      slope per watershed or flood-depth zone).

    Do NOT use this for: terrain FACE DIRECTION (use ``compute_aspect``); shaded
    relief (use ``compute_hillshade``); hypsometric color tint (use
    ``compute_colored_relief``); contour LINES (use ``compute_contours``);
    per-zone aggregation (use ``compute_zonal_statistics``).

    Honesty: a static single-time GDAL transform; land-surface DEM only (no
    bathymetry); raises a typed ``SlopeComputeError`` on failure -- never a fake
    layer.

    Action: the returned raster LayerURI auto-renders -- do not call
    publish_layer. Cached static-30d.

    Wraps ``gdaldem slope`` over a single-band elevation GeoTIFF; returns a
    Float32 slope raster in the input CRS and grid.

    Params:
        dem_uri: URI of a DEM GeoTIFF (typically ``fetch_dem(...).uri``, or
            ``fetch_copernicus_dem`` / ``fetch_3dep_extra``). Single-band, metres.
        output_unit: ``"degrees"`` (default, 0-90) for display; ``"percent"``
            (rise/run x 100) for road-grade / engineering contexts.
        algorithm: ``"Horn"`` (default) general-purpose 3x3 gradient;
            ``"ZevenbergenThorne"`` smoother on rough / noisy DEMs.

    Returns:
        A ``LayerURI`` (single-band Float32 raster) in the static-30d cache,
        same CRS and grid as the input DEM.

    FR-CE-8: routed through ``read_through`` -- repeat ``(dem_uri, output_unit,
    algorithm)`` calls reuse the cached raster (30-day TTL).

    Raises:
        SlopeComputeError: gdaldem unavailable / non-zero, or the DEM download
            fails. Carries ``error_code`` for the pipeline strip.
    """
