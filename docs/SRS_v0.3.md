...
**Decision Q: QGIS Processing is the primary agentic compute substrate** *(v0.3.24, per user direction 2026-06-16; supersedes the equal-status framing of §2.3 and Appendix E.)*
The QGIS Processing framework (native QGIS + GDAL / GRASS / SAGA / TauDEM providers and curated provider plugins) is the **default** surface through which the agent reaches geospatial and hazard-modeling compute. Algorithms are exposed to the LLM behind a generic **discover → describe → run** surface (`list_qgis_algorithms` / `describe_qgis_algorithm` / `qgis_process`, per FR-QS-1 / FR-AS-9), sourcing typed parameters and documentation from each algorithm's own metadata — so the agent uses battle-tested, documented, off-the-shelf algorithms instead of hand-rolled bespoke Python. Enabling a provider yields dozens of ready agentic tools without authoring wrappers. **Hand-rolled (Python-shim) atomic tools become the exception**, justified only when (a) no Processing algorithm covers the need, (b) license/runtime isolation requires an out-of-process container (NFR-L), or (c) a tight envelope-shaped result contract is cleaner authored directly; a new bespoke tool carries a one-line justification. External compute engines integrate as **headless solvers** (FR-CE handle contract) with QGIS Processing performing GIS pre-/post-processing — the SFINCS/HydroMT pattern generalized to HEC-HMS and the MODFLOW family. **QGIS Server is a REQUIRED component** for rendering + OGC parity (styled WMS/WFS, server-rendered vectors, print layouts); a dynamic COG tiler (TiTiler) MAY serve plain single-band raster overlays (flood depth, plume, precipitation) as a lightweight fast path and is the renderer of record for those layers' established look — it does **not** replace QGIS Server, which renders everything the COG tiler cannot (vectors, multi-layer symbology, layouts). The data-fetch tools and the external data endpoints they consume (OGC/ArcGIS/STAC) are independent of this decision and unchanged.
...
QGIS Server shall be deployed as a Cloud Run service, with autoscaling based on request rate. The container shall be based on the official `qgis/qgis-server` image, with additional QML styles and processing-algorithm-providing plugins (GRASS, SAGA, plus any hazard-relevant Processing plugins) baked in. The container shall additionally expose the `qgis_process` CLI to PyQGIS workers so that the agent's QGIS algorithm discovery (FR-AS-9, capability discovery Level 1a) can enumerate and invoke available Processing algorithms at runtime. *(v0.3.24, Decision Q.)* QGIS Server is a **REQUIRED** component — the renderer of record for styled, vector, and multi-layer/layout output and the OGC-parity surface — not an optional or deferrable convenience; a dynamic COG tiler (TiTiler) coexists only as the fast path for plain single-band raster overlays and does not replace it. The generic QGIS Processing **discover → describe → run** surface (FR-AS-9) is the agent's PRIMARY compute path. On the AWS deployment, QGIS Server + the headless `qgis_process` worker are re-hosted as containers (the Cloud Run phrasing above is the GCP-era target).
...
| 0.3.24 | 2026-06-16 | **Decision Q (§2.1): QGIS Processing is the primary agentic compute substrate** (per user direction 2026-06-16 — "the plan all along to utilize the built ecosystem around QGIS so we aren't hand-rolling tools that exist … off the shelf as agentic tools"). The generic discover→describe→run Processing surface (FR-AS-9 / FR-QS-1) is the DEFAULT compute path; hand-rolled Python-shim atomic tools become the exception (no covering algorithm / license isolation / envelope-contract clarity, each with a one-line justification). External compute engines integrate as headless solvers (FR-CE handle contract) with QGIS Processing doing GIS pre/post — the SFINCS/HydroMT pattern generalized to HEC-HMS + the MODFLOW family. **QGIS Server is REQUIRED** (rendering + OGC parity, server-rendered vectors, print layouts); **TiTiler is retained but scoped** to plain single-band COG overlays (flood / plume / precipitation — the renderer of record for that established look) and does NOT replace QGIS Server — correcting the sprint-14-aws TiTiler-only narrowing (which left vectors like NWS warnings mis-routed to a raster tiler → 500s). **§2.3 engine cloud-deployability gate**: a headless + Linux + scriptable solver ranks ahead of modeling sophistication for sequencing (HEC-HMS, MODFLOW, SFINCS qualify); **HEC-RAS deferred** (Windows-native + GUI-driven compute). **FR-QS-1 amended**: QGIS Server required + AWS-container re-host + Processing surface is the primary compute path. **Appendix E posture revised**: Processing-primary; `adapt-via-python` / `out-of-process` are the exceptions (plugin catalog itself unchanged). **New M12**: QGIS compute substrate on AWS + HEC-HMS proof. Execution plan: `reports/sprints/sprint-16-qgis-substrate-manifest.md` (jobs 0308–0313). Data-fetch tools + external data endpoints (OGC/ArcGIS/STAC) unchanged; Invariants 1 / 5 / 7 / 9 unchanged. |
