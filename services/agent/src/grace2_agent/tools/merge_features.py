...
logger = logging.getLogger("grace2_agent.tools.merge_features")
...
def _resolve_layer_to_local_path(uri: str, suffix: str) -> tuple[str, bool]:
    ...
    if uri.startswith("s3://"):
        ...
        with tempfile.NamedTemporaryFile(
            suffix=suffix, delete=False, prefix="grace2_merge_"
        ) as tmp:
            tmp.write(data)
...
def _merge_locally(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".fgb", delete=False, prefix="grace2_merge_out_"
        ) as out_f:
            tmp_out = out_f.name
