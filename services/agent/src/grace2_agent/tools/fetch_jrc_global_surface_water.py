...
Fetches the **JRC Global Surface Water** (GSW) long-term surface-water statistics
for a bbox via the Microsoft Planetary Computer (PC) STAC catalog (collection
``jrc-gsw``) and returns a single-band uint8 COG with a band-appropriate color
ramp baked in, so ``publish_layer``'s embedded-palette passthrough colorizes it
directly.

The Joint Research Centre (European Commission) derived GSW from 38 years
(1984-2021, v1.4 / 1984-2020 PC mosaic) of Landsat imagery: at 30 m it classifies
every observation as water or not, then reduces the full record to per-pixel
statistics. This tool surfaces the four most useful continuous statistics:

    occurrence    0..100 %  frequency water was present over the whole record
                            (the FLOOD-FREQUENCY / where-water-ever-was layer)
    recurrence    0..100 %  inter-annual recurrence: of the years water occurred
                            at all, the % in which it returned (seasonal lakes
                            ~ low, permanent water ~ 100)
    seasonality   0..12     number of months per year water was present
                            (12 = permanent, 1..11 = seasonal, 0 = no water)
    change        0..200    intensity of change in occurrence between the two
                            epochs (1984-1999 vs 2000-2021); 100 = no net change,
                            < 100 = loss, > 100 = gain (253 = not water / nodata)

Why GSW (vs the NDWI ``digitize_water_body`` tool)?  ``digitize_water_body``
outlines water in ONE recent scene; GSW is the 38-year BASELINE  --  it answers
"is this water permanent or seasonal?", "where has water EVER been (the flood
envelope)?" and "is this lake shrinking or growing?". The two complement each
other: a long-term baseline plus a current snapshot.

Data source
===========

PC collection ``jrc-gsw`` (JRC Global Surface Water, 30 m, global):

    catalog: https://planetarycomputer.microsoft.com/api/stac/v1
    assets:  occurrence / recurrence / seasonality / change (+ extent /
             transitions, not surfaced here), each a uint8 single-band COG tiled
             on a 10x10-degree global grid (items like ``100W_40Nv1_3_2020``).

A bbox can straddle adjacent 10-degree tiles, so we select EVERY ``jrc-gsw``
item whose footprint intersects the bbox, warp+window-read the requested band of
each to EPSG:4326 at 30 m, and mosaic them (first valid pixel wins). The output
COG carries a baked GDAL color table on band 1 so the layer is self-styling.

Assets are Azure-Blob COGs behind SAS tokens. Unlike the per-collection token
path the other PC tools use, the ``jrc-gsw`` per-collection token is not accepted
by the blob (returns AuthenticationFailed), so this tool signs each asset href
through the PC ``/sign`` REST endpoint (which signs an arbitrary blob href and is
the canonical path the official ``planetary-computer`` SDK uses) and reads
through GDAL ``/vsicurl/``.

Rendering
=========

The output is a single-band uint8 COG with a band-specific GDAL color table
stamped on band 1 (``write_colormap`` + ``ColorInterp.palette``).
``publish_layer``'s ``_resolve_titiler_style_params`` step 1 detects the embedded
palette and returns empty style params, so TiTiler colorizes from the baked ramp
with NO rescale/colormap override (the same registry-independent path NLCD
``fetch_landcover`` / ``fetch_esri_landcover_10m`` rely on). ``occurrence`` and
``recurrence`` get a white->blue ramp (0 transparent), ``seasonality`` a
12-step blue ramp, ``change`` a red(loss)->white(no change)->blue(gain) diverging
ramp. The ``style_preset`` is a descriptive token (e.g.
``water_occurrence_pct``); because the embedded palette wins, the token never
reaches the single-band rescale registry.

Honesty (data-source fallback norm): if NO item covers the bbox, or every mosaic
pixel is no-data (an ocean / fully-dry AOI), a typed
``JrcSurfaceWaterNoCoverageError`` is raised  --  never a fabricated layer.

FR-CE-8 / FR-DC-3/4: routed through ``read_through`` so identical ``(bbox, band)``
calls reuse the cached COG in the ``static-30d`` / ``jrc_global_surface_water``
cache prefix.
...
logger = logging.getLogger("grace2_agent.tools.fetch_jrc_global_surface_water")
...
def _fetch_surface_water_cog_bytes(
    ...
    try:
        with tempfile.NamedTemporaryFile(
            suffix=".tif", delete=False, prefix="grace2_jrc_gsw_"
        ) as f:
            tmp_path = f.name
        profile = dict(
            driver="COG",
