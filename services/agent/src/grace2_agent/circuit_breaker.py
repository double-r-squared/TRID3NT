...
logger = logging.getLogger("grace2_agent.circuit_breaker")
...
def _get_threshold() -> int:
    ...
    Override via ``GRACE2_CIRCUIT_THRESHOLD`` (integer).  Falls back to 3.
    """
    raw = os.environ.get("GRACE2_CIRCUIT_THRESHOLD")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "GRACE2_CIRCUIT_THRESHOLD=%r is not a valid positive integer; "
            "using default %d",
            raw,
            _DEFAULT_THRESHOLD,
        )
...
def _get_cooldown_s() -> float:
    ...
    Override via ``GRACE2_CIRCUIT_COOLDOWN_S`` (float).  Falls back to 60.0.
    """
    raw = os.environ.get("GRACE2_CIRCUIT_COOLDOWN_S")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "GRACE2_CIRCUIT_COOLDOWN_S=%r is not a valid float; "
            "using default %.1f",
            raw,
            _DEFAULT_COOLDOWN_S,
        )
