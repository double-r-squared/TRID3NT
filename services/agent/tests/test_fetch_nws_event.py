...
_LIVE_NWS = os.environ.get("GRACE2_TEST_LIVE_NWS") == "1"
...
def test_user_agent_header_sent_on_request():
    ...
    with patch("grace2_agent.tools.fetch_nws_event.httpx.Client", FakeClient):
        from grace2_agent.tools.fetch_nws_event import _fetch_nws_geojson
...
def test_fl_state_response_with_three_alerts_writes_fgb():
    ...
    with patch(
        "grace2_agent.tools.fetch_nws_event._fetch_nws_geojson",
        return_value=fake_geojson,
    ), patch(
        "grace2_agent.tools.fetch_nws_event.read_through",
...
def test_cache_miss_invokes_fetch_fn_then_hit_skips():
    ...
    with patch(
        "grace2_agent.tools.fetch_nws_event._fetch_nws_event_bytes",
...
def test_geojson_to_fgb_empty_collection_is_valid():
    ...
    from grace2_agent.tools.fetch_nws_event import _geojson_to_fgb
...
# ---------------------------------------------------------------------------
# Live integration test (GRACE2_TEST_LIVE_NWS=1 to run).
# ---------------------------------------------------------------------------


@pytest.mark.skipif(
    not _LIVE_NWS,
    reason="Set GRACE2_TEST_LIVE_NWS=1 to run live NWS api.weather.gov tests",
)
